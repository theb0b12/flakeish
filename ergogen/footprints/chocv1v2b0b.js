...
//
// Authors: @ergogen + @infused-kim, @ceoloide, @grazfather, @nxtk improvements, @theb0b
//
// Description:
...
// 
// @theb0b's improvements:
// - moved around the reversible copper traces to make it so they work with the gateron ks-27/33 footprint
...
module.exports = {
  ...
  (footprint "ceoloide:switch_choc_v1_v2"
    ...
    (segment
        (start ${p.eaxy(-2.854, -1.896)})
        (end ${p.eaxy(-1.475, -3.275)})
        (width ${p.trace_width})
        (locked ${p.locked_traces_vias ? 'yes' : 'no'})
        (layer "F.Cu")
        (net ${p.to.index})
    ...
    (segment
        (start ${p.eaxy(-1.475, -3.275)})
        (end ${p.eaxy(-0.975, -3.275)})
        (width ${p.trace_width})
        (locked ${p.locked_traces_vias ? 'yes' : 'no'})
        (layer "F.Cu")
        (net ${p.to.index})
    ...
    (segment
        (start ${p.eaxy(-0.975, -3.275)})
        (end ${p.eaxy(0.961166, -3.275)})
        (width ${p.trace_width})
        (locked ${p.locked_traces_vias ? 'yes' : 'no'})
        (layer "B.Cu")
        (net ${p.to.index})
    ...
    (segment
        (start ${p.eaxy(0.961166, -3.275)})
        (end ${p.eaxy(1.461166, -3.275)})
        (width ${p.trace_width})
        (locked ${p.locked_traces_vias ? 'yes' : 'no'})
        (layer "B.Cu")
        (net ${p.to.index})
